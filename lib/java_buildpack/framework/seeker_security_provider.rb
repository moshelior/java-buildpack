  ...
  module Framework
    ...
    class SeekerSecurityProvider < JavaBuildpack::Component::BaseComponent
      ...
      def compile
        ...
        download_zip('', URI.join(credentials[ENTERPRISE_SERVER_URI_SERVICE_CONFIG_KEY],SENSOR_ZIP_RELATIVE_PATH_AT_ENTERPRISE_SERVER), false, sensor_dir)
        shell "unzip -qq #{File.join(sensor_dir,'SensorInstaller.zip')} -d #{@droplet.sandbox} 2>&1"
        shell "unzip -qq #{File.join(sensor_dir,'SeekerInstaller.jar')} #{AGENT_JARS_RELATIVE_PATH_IN_JAR} -d #{sensor_dir} 2>&1"
        shell "rm -rf #{sensor_dir}"
      ...
      def assert_configuration_valid(credentials)
        mandatory_config_keys =
          [ENTERPRISE_SERVER_URI_SERVICE_CONFIG_KEY, SEEKER_HOST_SERVICE_CONFIG_KEY, SEEKER_HOST_PORT_SERVICE_CONFIG_KEY]
      ...
      AGENT_JARS_RELATIVE_PATH_IN_JAR ='inline/agents/java/'
      ...
      private_constant :SEEKER_HOST_SERVICE_CONFIG_KEY, :SEEKER_HOST_PORT_SERVICE_CONFIG_KEY,
                       ...
                       :AGENT_JARS_RELATIVE_PATH_IN_JAR
    end
  end
end
